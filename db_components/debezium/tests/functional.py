...
class DebeziumCDCDatadirTest(TestDataDir):
    ...
    @staticmethod
    def _remove_column_slice(manifest_path: str, table_path: str, drop_columns: list[str], order_by_columns: list[str]):
        ...
        order_by_columns = ','.join([f'"{column}"' for column in order_by_columns])

        # Write the DataFrame back to a CSV file
        duckdb.execute(f"COPY (SELECT {select_columns} FROM {table_name} ORDER BY {order_by_columns} ASC) "
    ...
    def _cleanup_result_data(self):
        ...
        for in_table in in_tables:
            ...
            order_by_columns = traits.get_order_by_columns(Path(in_table).stem)
